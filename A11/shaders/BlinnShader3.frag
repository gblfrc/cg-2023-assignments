...
void main() {
	...
	vec3 l = vec3(gubo.lightColor);
	vec3 d = normalize(gubo.lightDir);
	vec3 lightDir = normalize(p-x);
	float cosalpha = dot(lightDir, d);
	float clampTerm = (cosalpha-cosout)/(cosin-cosout);
	vec3 pointLightTerm = l*pow((g/length(p-x)),beta);
	vec3 lightColor = pointLightTerm*clamp(clampTerm, 0.0, 1.0);
